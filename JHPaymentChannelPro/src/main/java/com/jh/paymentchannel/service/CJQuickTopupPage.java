...
@Service
public class CJQuickTopupPage extends BaseChannel implements TopupRequest {
	...
	@Autowired
	private CJQuickpageRequest cjquickpageRequest;
	...
	public Map<String, String> topupRequest(Map<String, Object> params) throws Exception {
		...
		if ("10".equals(orderType)) {
			...
			if (!rate.equals(cjhkRegister.getRates())) {

				map = (Map<String, String>) cjquickpageRequest.cjhkUpdateMerchant(request, ordercode);
				...
				if ("000000".equals(respCode)) {
					...
					if (!extraFee.equals(cjhkRegister.getExtraFees())) {

						map = (Map<String, String>) cjquickpageRequest.cjQuickUpdateWithDrawFee(request, ordercode);
						...
						if ("000000".equals(respCode)) {
							...
							map = (Map<String, String>) cjquickpageRequest.cjQuickFastPay(request, ordercode);
						}
			...
			} else if (!extraFee.equals(cjhkRegister.getExtraFees())) {
		...
		if ("11".equals(orderType)) {
			...
			map = (Map<String, String>) cjquickpageRequest.cjhkTransfer(request, ordercode, extra);
