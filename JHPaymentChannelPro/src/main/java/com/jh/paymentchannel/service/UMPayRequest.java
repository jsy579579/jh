...
@Service
public class UMPayRequest implements PayRequest{
	...
	public WithDrawOrder payRequest(String ordercode,
			String cardno, String username, String amount, String bankname, String phone,  String priOrpub,String notifyURL,String returnURL) {
				...
				//摘要【purpose】
				map.put("purpose",username);
				//开户行支行全称【bank_brhname】
				map.put("bank_brhname","对私");
				//付款实时和非实时标识【checkFlag】
				map.put("checkFlag","0");
				//收款方手机号【mobile_no】
...
//					log.info("联动提现请求地址="+get_url);
//					log.info("联动提现请求签名="+reqDataGet.getSign());
//					drawOrder.setResurl(get_url);
//					RestTemplate restTemplate=new RestTemplate();
//					log.info("联动提现请求1");
//					String resultStr = restTemplate.getForObject(get_url, String.class);//(get_url,  String.class, reqDataPost.getField());//;
//					log.info("联动提现请求2");
//					String responseCode = resultStr;//resultStr.getBody();
//					log.info("联动提现请求返回="+responseCode);
//					responseCode= responseCode.substring(responseCode.indexOf("CONTENT=\"")+9,responseCode.length());
//					responseCode= responseCode.substring(0,responseCode.indexOf("\">"));
//					log.info("联动提现请求返回="+responseCode);
//					Map<String, String>  dataMap=new HashMap<String, String>();
//					dataMap=getMap(responseCode);
