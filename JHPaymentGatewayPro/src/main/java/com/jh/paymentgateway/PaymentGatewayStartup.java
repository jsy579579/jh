...
@EnableAsync
@EnableScheduling
public class PaymentGatewayStartup extends WebMvcConfigurerAdapter {

	public static void main(String[] args) throws Exception {
		List<String> params = new ArrayList<>();
		StringBuffer argsSB = new StringBuffer();
		String argsS = "";
		String logPath = "logs/paymentgateway.log";;
		for (String arg : args) {
			argsSB.append(arg+";");
			params.add(arg);
		...
		if (!argsS.contains("logFileName=")) {
			System.setProperty("logFileName", "paymentgateway");
		}else {
			for (String string : params) {
				if (string.contains("logFileName=")) {
					System.setProperty("logFileName", string.substring(string.indexOf("=")+1, string.length()));
				}
			}
		}

		args = params.toArray(new String[params.size()]);
//		new SpringApplicationBuilder(PaymentGatewayStartup.class).properties(propertiesMap).run(args);
		SpringApplication.run(PaymentGatewayStartup.class, args);
	}
