 ...
 */
@Service
public class CJHKLRTopupPage extends BaseChannel implements TopupRequestBusiness {
	...
	private static final Logger LOG = LoggerFactory.getLogger(CJHKLRTopupPage.class);
	...
	public Map<String, Object> topupRequest(Map<String, Object> params) throws Exception {
		...
		if ("10".equals(orderType)) {
			LOG.info("判断进入消费任务==============");
			// 判断用户是否需要修改费率和固定手续费
			if (!rate.equals(cjhklr.getRate()) | !extraFee.equals(cjhklr.getExtraFee())) {
				maps = (Map<String, Object>) cjhkTopupRequest.cjhkChangeRate(idCard, rate, "110002", extraFee);
				if (!"000000".equals(maps.get("resp_code"))) {
					LOG.info("======================修改渠道:110002==============执行失败");
					return maps;
				}
				maps = (Map<String, Object>) cjhkTopupRequest.cjhkChangeRate(idCard, rate, "101002", extraFee);
				if (!"000000".equals(maps.get("resp_code"))) {
					LOG.info("======================修改渠道:101002==============执行失败");
					return maps;
				}
			}
