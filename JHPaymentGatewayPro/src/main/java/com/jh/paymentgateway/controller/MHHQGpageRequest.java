...
public class MHHQGpageRequest  extends BaseChannel {
	private static final Logger LOG = LoggerFactory.getLogger(MHHQGpageRequest.class);
	...
	// 跟还款对接的接口
		@RequestMapping(method = RequestMethod.POST, value = "/v1.0/paymentgateway/topup/mhhqg/torepayment")
		public @ResponseBody Object HLJCRegister(HttpServletRequest request,HttpServletResponse response,
			...
			MHHQGBindCard hqBindCard = topupPayChannelBusiness.getMHHQGBindCardByBankCard(bankCard);
			
			if (hqRegister == null||!hqRegister.getStatus().equals("1")) {
				return ResultWrap.init("999996", "需要开户",ip+"/v1.0/paymentgateway/topup/mhhqg/registetpage?bankCard="+bankCard+
				...
				if (hqBindCard == null || !"1".equals(hqBindCard.getStatus())) {
					map=ResultWrap.init("999996", "需要绑卡",ip+ "/v1.0/paymentgateway/topup/mhhqg/bindCard?bankCard="+bankCard
	...
	// 跳转到开户页面
	@RequestMapping(method = RequestMethod.GET, value = "/v1.0/paymentgateway/topup/mhhqg/registetpage")
	public String  	HQGRegisterpage(HttpServletRequest request, HttpServletResponse response,
		...
		return "mhhqgregistet";
	}
	
	//开户接口
	@RequestMapping(method = RequestMethod.POST, value = "/v1.0/paymentgateway/topup/mhhqg/registet")
	public  @ResponseBody Object  	HQGRegister(HttpServletRequest request, HttpServletResponse response,
		...
		hqRegister = topupPayChannelBusiness.getMHHQGRegisterByIdCard(idCard);
		if(hqRegister!=null &&hqRegister.getStatus().equals("1")){
			if(hqBindCard==null||!hqBindCard.getStatus().equals("1")){
				returnMap=ResultWrap.init(CommonConstants.SUCCESS,"请求成功" ,
						ip+ "/v1.0/paymentgateway/topup/mhhqg/bindCard?bankCard="+bankCard+"&idCard="+idCard+"&phone="+phone+"&userName="+userName )   ;
			}
		}else{
	...
	//修改费率
	@RequestMapping(method = RequestMethod.POST, value = "/v1.0/paymentgateway/topup/mhhqg/registetpage")
	...
	//绑卡接口
	@RequestMapping(method = RequestMethod.GET, value = "/v1.0/paymentgateway/topup/mhhqg/bindCard")
	public String HQGbindCard(HttpServletRequest request, HttpServletResponse response,
		...
		try {
			Map<String,String> bindCard=HQGbindCard(merchantOrder,hqRegister.getMerchantCode(), bankCard, userName, idCard, phone, userIP, 
					ip+"/v1.0/paymentgateway/topup/hqg/bindcard/front", ip+"/v1.0/paymentgateway/topup/mhhqg/opencard/notifyurl");
			String returncode=bindCard.get("returncode");
			if(returncode.equals("0000")){
				if(hqBindCard==null){
					hqBindCard=new MHHQGBindCard();
				...
				topupPayChannelBusiness.createMHHQGBindCard(hqBindCard);
	...
	//消费接口
	@RequestMapping(method = RequestMethod.GET, value = "/v1.0/paymentgateway/topup/mhhqg/fastpay")
	public @ResponseBody Object hqFastPay(HttpServletRequest request,@RequestParam(value = "orderCode") String orderCode) {
		...
		Map<String,String> hqgPay=HQGPay(orderCode,hqRegister.getMerchantCode(), amount, bankCard, deviceId, userIP, mcc, ip + "/v1.0/paymentgateway/topup/mhhqg/fastpay/notifyurl");
	...
	//代付接口
	@RequestMapping(method = RequestMethod.POST, value = "/v1.0/paymentgateway/topup/mhhqg/transfer")
	...
	//余额查询接口
	@RequestMapping(method = RequestMethod.POST, value = "/v1.0/paymentgateway/topup/mhhqg/balancequery")
	...
	//订单查询接口
	@RequestMapping(method = RequestMethod.POST, value = "/v1.0/paymentgateway/topup/mhhqg/orderquery")
	...
	//同名卡开卡异步通知
	@RequestMapping(method = { RequestMethod.POST,
			RequestMethod.GET }, value = "/v1.0/paymentgateway/topup/mhhqg/opencard/notifyurl")
	public void OpenCardBack(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		MHHQGBindCard hqBindCard = topupPayChannelBusiness.getMHHQGBindCardbyMerchantOrder(dsorderid);
		if (status.equals("00")) {
	...
	//同名卡交易异步通知
	@RequestMapping(method = { RequestMethod.POST,
			RequestMethod.GET }, value = "/v1.0/paymentgateway/topup/mhhqg/fastpay/notifyurl")
...
//		String merchantOrder = UUID.randomUUID().toString().replaceAll("-", "");
	...
	//手动代付
	@RequestMapping(method = RequestMethod.POST, value = "/v1.0/paymentgateway/topup/mhhqg/transfer/manual")
