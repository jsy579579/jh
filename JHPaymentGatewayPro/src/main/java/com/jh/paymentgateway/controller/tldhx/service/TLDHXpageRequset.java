package com.jh.paymentgateway.controller.tldhx.service;
...
import com.jh.paymentgateway.controller.tldhx.dao.TLDHXBusiness;
...
public class TLDHXpageRequset extends BaseChannel {

    @Autowired
    private RedisUtil redisUtil;

    @Autowired
    private NewTopupPayChannelBusiness topupPayChannelBusiness;

    @Autowired
    private TLDHXBusiness tldhxbusiness;
    ...
    private static final Logger LOG = LoggerFactory.getLogger(TLDHXpageRequset.class);
    protected static final Charset UTF_8 = StandardCharsets.UTF_8;
    private static String key = "75D502AAB716281B5532DE8C9BA33436";
    private static String partnerNo = "OOxD5hbO";
    private static String requestURL = "http://fast.jfpays.com:19085/rest/api/";

    // 与还款对接
    @RequestMapping(method = RequestMethod.POST, value = "/v1.0/paymentgateway/repayment/tldhx/Dockentrance")
    public @ResponseBody Object Dockentrance(@RequestParam(value = "dbankCard") String deditBankCard,
                                             ...
                                             @RequestParam(value = "dphone") String debitPhone,
                                             @RequestParam(value = "userName") String userName,
                                             @RequestParam(value = "dbankName") String debitBankName,
                                             @RequestParam(value = "rate") String rate,
                                             @RequestParam(value = "extraFee") String extraFee,
                                             @RequestParam(value = "securityCode") String securityCode,
                                             @RequestParam(value = "expiredTime") String expiredTimes,
                                             @RequestParam(value = "cardtype") String cardtype) throws Exception {

        String expiredTime=this.expiredTimeToMMYY(expiredTimes);
        LOG.info("通联小额代还请求参数==========dbankCard=="+deditBankCard+"====dphone======="+debitPhone+"=======dbankName========"+debitBankName);
        // 获取银行联行号
//        BankNumCode bcode = topupPayChannelBusiness.getBankNumCodeByBankName(bankName);
        ...
        List<CheckedBank> all = checkedBankRepsository.findAll();
        int i = (int) all.stream().filter(item -> item.equals(cardtype)).count();
        if(i>0){
            return ResultWrap.init(CommonConstants.FALIED, "该通道不支持准贷记卡");
        }
        String bankUnitNo = bcode.getBankCode();
//        String bankUnitNo="104100000004";
        Map<String, Object> maps = new HashMap<String, Object>();

        TLDHXHHRegister tldhxhhRegister = topupPayChannelBusiness.getTLDHXRegisterByIdCard(idCard);
        TLDHXHHBindCard tldhxhhBindCard = topupPayChannelBusiness.getTLDHXBindCardByBankCard(bankCard);

        if (tldhxhhRegister == null) {
            maps = (Map<String, Object>) this.register(deditBankCard, idCard, debitPhone, userName, rate, extraFee,bankUnitNo,debitBankName);
            LOG.info("通联小额进件结果============="+maps.toString());
            if(!CommonConstants.SUCCESS.equals(maps.get(CommonConstants.RESP_CODE))){
                return maps;
            }

        }else if (!extraFee.equals(tldhxhhRegister.getExtraFee()) | !bankCard.equals(tldhxhhRegister.getBankCard()) | !rate.equals(tldhxhhRegister.getRate())) {
        ...
        if (tldhxhhBindCard == null || tldhxhhBindCard.getStatus().equals("0")) {
            LOG.info("===================用户未绑卡，开始绑卡===========================");
            maps = ResultWrap.init("999996", "需要绑卡",
                    ip + "/v1.0/paymentgateway/repayment/tldhx/bindcard?bankCard=" + bankCard
    ...
    通联小额进件
     */
    @RequestMapping(method = RequestMethod.POST,value ="/v1.0/paymentgateway/quick/tldhx/register" )
    public @ResponseBody Object register(@RequestParam("bankCard")String bankCard,
        ...
        rateList.put("QUICKPAY_NOSMS", rate);
        // 公共参数
        ...
        try {
            ...
            if (!"".equals(platMerchantCode) && platMerchantCode != null) {
                TLDHXHHRegister tldhxhhRegister = new TLDHXHHRegister();
                tldhxhhRegister.setBankCard(bankCard);
                tldhxhhRegister.setExtraFee(extraFee);
                tldhxhhRegister.setRate(rate);
                tldhxhhRegister.setMerchantNo(platMerchantCode);
                tldhxhhRegister.setIdCard(idCard);
                tldhxhhRegister.setPhone(phone);
                topupPayChannelBusiness.createTLDHXRegister(tldhxhhRegister);
                map.clear();
    ...
    // 跳转到绑卡页面
    @RequestMapping(method = RequestMethod.GET, value = "/v1.0/paymentgateway/repayment/tldhx/bindcard")
    ...
    @RequestMapping(method = RequestMethod.POST, value = "/v1.0/paymentgateway/repayment/tldhx/bindcardsms")
    public @ResponseBody Object OpenSMS(@RequestParam(value = "bankCard") String bankCard,
        ...
        // 获取平台商户号
        TLDHXHHRegister tl = topupPayChannelBusiness.getTLDHXRegisterByIdCard(idCard);
        String MerchantNo = tl.getMerchantNo();
        ...
        try {
            ...
            if ("000000".equals(respCode)) {
                ...
                try {
                    ...
                    TLDHXHHBindCard tldhxhhBindCard = topupPayChannelBusiness.getTLDHXBindCardByBankCard(bankCard);
                    if (tldhxhhBindCard==null){
                        tldhxhhBindCard = new TLDHXHHBindCard();
                        tldhxhhBindCard.setBankCard(bankCard);
                        tldhxhhBindCard.setIdCard(idCard);
                        tldhxhhBindCard.setPhone(phone);
                        tldhxhhBindCard.setStatus("0");
                        tldhxhhBindCard.setBindingNum(openOrderId);
                        if (openCardId!=null&&!("").equals(openCardId)) {
                            LOG.info("支付卡号为=======" + openCardId);
                            tldhxhhBindCard.setOpenCardid(openCardId);
                        }
                        topupPayChannelBusiness.createTLDHXBindCard(tldhxhhBindCard);
                    }
                    ...
                    e.printStackTrace();
                    LOG.info("通联小额开通支付卡短信=================未返回openCardId");
                }
                maps.put(CommonConstants.RESP_CODE, CommonConstants.SUCCESS);
                maps.put(CommonConstants.RESP_MESSAGE, message);
                ...
                maps.put(CommonConstants.RESP_MESSAGE, message);
                LOG.info("通联小额开通支付卡短信---异常：" + message);
//                this.addOrderCauseOfFailure(orderCode, message, rip);
     ...
     * @return
     */
    @RequestMapping(method = RequestMethod.POST, value ="/v1.0/paymentgateway/repayment/tldhx/confirmSms")
    public @ResponseBody Object OpenPaymentCard(@RequestParam(value = "bankCard") String bankCard,
                                                @RequestParam(value = "smsCode") String smsCode,
                                                @RequestParam(value = "idCard") String idCard) {


        TLDHXHHBindCard tlB = topupPayChannelBusiness.getTLDHXBindCardByBankCard(bankCard);
        String openOrderId= tlB.getBindingNum();
        String phone=tlB.getPhone();
     ...
     * @return
     */
    @RequestMapping(method = RequestMethod.POST, value ="/v1.0/paymentgateway/repayment/tldhx/jfpay")
    public @ResponseBody Object toPay(@RequestParam(value = "ordercode") String orderCode) {
        ...
        LOG.info("进入通联小额消费=========="+prp.toString());
        ...
        // 获取开卡机构号
        TLDHXHHBindCard tldhxhhBindCard = topupPayChannelBusiness.getTLDHXBindCardByBankCard(bankCard);
        String openCardId = tldhxhhBindCard.getOpenCardid();
...
//        String orderId = getRandom();
        ...
        map.put("remark", "大额代还");
        map.put("productCode", "QUICKPAY_NOSMS");
        map.put("notifyUrl", ip + "/v1.0/paymentgateway/repayment/tldhx/payNotifyUrl");
        ...
        try{
            ...
            if (com.google.common.base.Objects.equal(signature, checkSign)) {
                ...
                if ("01".equals(orderStatus)) {
                    ...
                    if("TLDHXHH_QUICK".equals(channelTag)){
                        version="54";
                    }
     ...
     * @return
     */
    @RequestMapping(method = RequestMethod.POST, value ="/v1.0/paymentgateway/repayment/tldhx/balanceQuery")
...
//        int i=0;
//        int c=0;
//        List<TLDHXMerchant> lists=jfdEbusiness.getAllProvincial();
//        for (TLDHXMerchant jfdeMerchant:lists) {
//          String ciyt=jfdeMerchant.getCity();
//            System.out.println("该城市是"+ciyt);
//        TLDHXArea jfdeArea=jfdEbusiness.getmccBycity(ciyt);
//        if (jfdeArea==null){
...
//    }
    ...
    @RequestMapping(method = RequestMethod.POST,value = "/v1.0/paymentgateway/repayment/tldhx/choosemerchant")
    public @ResponseBody Object merchant(@RequestParam(value = "area",required = false,defaultValue = "-1")String area,
        ...
        if (area.equals("-1") && provincial.equals("-1")) {
            List<TLDHXMerchant> lists=tldhxbusiness.getAllProvincial();
        ...
        if (!"-1".equals(area)) {
            TLDHXArea result=tldhxbusiness.getAllByArea(area);
            if (result!=null ){
                maps.put(CommonConstants.RESP_CODE,CommonConstants.SUCCESS);
        ...
        if (!"-1".equals(provincial)) {
            List<TLDHXMerchant> result=tldhxbusiness.getAllByProvincial(provincial);
    ...
    @RequestMapping(method = RequestMethod.POST,value = "/v1.0/paymentgateway/repayment/tldhx/choosemcc")
    public @ResponseBody Object merchant(@RequestParam(value = "mcc",required = false,defaultValue = "-1")String mcc){
        Map<String,Object> maps=new HashMap<>();
        List<TLDHXMcc> lists=tldhxbusiness.getAllMcc();
        maps.put(CommonConstants.RESP_CODE,CommonConstants.SUCCESS);
        maps.put(CommonConstants.RESP_MESSAGE,lists);
        return maps;
    }
    ...
    public Object modifyCard(String bankCard, String phone, String rate, String ex, String idCard) {
        TLDHXHHRegister tldhxhhRegister = topupPayChannelBusiness.getTLDHXRegisterByIdCard(idCard);
        String MerchantNo = tldhxhhRegister.getMerchantNo();
        ...
        try {
            ...
            if ("000000".equals(respCode)) {
                tldhxhhRegister.setBankCard(bankCard);
                tldhxhhRegister.setExtraFee(ex);
                tldhxhhRegister.setRate(rate);
                tldhxhhRegister.setCreateTime(new Date());
                topupPayChannelBusiness.createTLDHXRegister(tldhxhhRegister);
                maps.put(CommonConstants.RESP_CODE, CommonConstants.SUCCESS);
                maps.put(CommonConstants.RESP_MESSAGE, message);
            } else {
     ...
     */
    @RequestMapping(method = { RequestMethod.POST,
            RequestMethod.GET }, value = "/v1.0/paymentgateway/repayment/tldhx/settleNotifyUrl")
    public void openFront(HttpServletRequest request, HttpServletResponse response) throws IOException {
        LOG.info("[通联小额还款]进入还款回调================================");
     ...
     * @return
     */
    @RequestMapping(method = RequestMethod.POST, value ="/v1.0/paymentgateway/repayment/tldhx/conQuery")
     ...
     * 付款61000
     */
    @RequestMapping(method = RequestMethod.POST, value ="/v1.0/paymentgateway/repayment/tldhx/toreppay")
    public @ResponseBody Object toSettle(@RequestParam(value = "orderCode") String orderCode) {
        ...
        String rip = prp.getIpAddress();
        String  extra   = prp.getExtra();
        LOG.info("进入通联小额还款=========="+prp.toString());
        ...
        map.put("remark", "通联小额代还");
        map.put("productCode", "QUICKPAY_NOSMS");
        map.put("backUrl", ip + "/v1.0/paymentgateway/repayment/tldhx/settleNotifyUrl");
     ...
     * @return
     */
    @RequestMapping(method = RequestMethod.POST, value ="/v1.0/paymentgateway/repayment/tldhx/reppayQuery")
