...
public class GetSpToken {
	...
	public BaseResMessage<TokenRes> token(String key, String spCode) throws IOException {
		...
		BaseResMessage<JSONObject> res = null;
		BaseResMessage<TokenRes> respToken = new BaseResMessage<>();
		if (response.isSuccessful()) {
			res = JSONObject.parseObject(tokenJsonRsp, new TypeReference<BaseResMessage<JSONObject>>(){});
			JSONObject data = res.getData();
			TokenRes tokenRes = new TokenRes();
			tokenRes.setToken(data.getString("token"));
			respToken.setCode(res.getCode());
			respToken.setData(tokenRes);
			respToken.setMessage(res.getMessage());
		} else {
