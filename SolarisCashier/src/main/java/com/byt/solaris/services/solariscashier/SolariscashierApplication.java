package com.byt.solaris.services.solariscashier;
...
public class SolariscashierApplication extends WebMvcConfigurerAdapter {

    public static void main(String[] args) throws Exception {
        List<String> params = new ArrayList<>();
        StringBuffer argsSB = new StringBuffer();
        String argsS = "";
        String logPath = "logs/SolarisCashier.log";
        for (String arg : args) {
            argsSB.append(arg+";");
            params.add(arg);
...
//		new SpringApplicationBuilder(CreditCardManagerStartUp.class).properties(propertiesMap).run(args);
        SpringApplication.run(SolariscashierApplication.class, args);
        Properties logProp = new Properties();
        logProp.load(SolariscashierApplication.class.getClassLoader().getResourceAsStream("log4j.properties"));
